...
let sounds = [];
let soundsrc = [
    'assets/electricity01.m4a',
    'assets/electricity02.m4a',
    'assets/electricity03.m4a',
    'assets/electricity04.m4a',
]
...
function initAudio() {
    ...
    audioLoader.load('assets/sun_sonification.wav', function (buffer) {
...
function addProton() {
    ...
    let index = Math.floor(randFloat(0, 4));
    let audioLoader = new THREE.AudioLoader();
    audioLoader.load(soundsrc[index], function (buffer) {
        let esound = new THREE.PositionalAudio(listener);
        esound.setBuffer(buffer);
        esound.setLoop(true);
        esound.setRefDistance(20);
        esound.setVolume(0.1);
        proton.mesh.add(esound);
        proton.mesh.children[0].play();
    });
...
function addElectron() {
    ...
    audioLoader.load(soundsrc[index], function (buffer) {
        ...
        esound.setRefDistance(20);
        esound.setVolume(0.1);
        electron.mesh.add(esound);
        electron.mesh.children[0].play();
    });
...
function checkAudio() {
    if (protons.length > 0 || electrons.length > 0) {

        if (!playSound) {

            playSound = true;
            playAudio();
            console.log('playsound is');
...
function playAudio() {
    ...
    if (playSound && !sound.isPlaying) {
        ...
        sound.play();
    } else if (!playSound && sound.isPlaying) {
        sound.stop();
...
function render() {
    ...
    if (electrons.length > 0) {
        ...
        for (let i = 0; i < electrons.length; i++) {
            ...
            if (electrons[i].finished) {
                ...
                electrons[i].mesh.children[0].stop();
    ...
    let totalAtoms = electrons.length + protons.length;
    let volumne = mapLinear(totalAtoms, 0, 20, 0.2, 0.8);

    if (electrons.length > 0) {

        for (let i = 0; i < electrons.length; i++) {
            electrons[i].mesh.children[0].setVolume(randFloat(volumne - 0.1, volumne + 0.1));
    ...
    if (protons.length > 0) {
        ...
        for (let i = 0; i < protons.length; i++) {
            protons[i].mesh.children[0].setVolume(randFloat(volumne - 0.1, volumne + 0.1));
        }
...
class Proton {
    constructor(scene) {
        ...
        this.pos = new THREE.Vector3(randFloatSpread(cubeDim), randFloatSpread(cubeDim / 10), randFloatSpread(0));
    ...
    update() {
        ...
        // this.pos.x += randFloatSpread(0.1);
        // this.pos.y += randFloatSpread(0.1);
        // this.pos.z += randFloatSpread(0.1);
